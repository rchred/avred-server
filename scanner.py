...
def delete_file(filepath):
	try:
		...
		logging.info(f"Could not delete {filepath}, it's probably held by some process.")
...
def save_file(data, filepath):
	try:
		...
		logging.info(f"Could not save {filepath}! Exception: {str(e)}")
		return False
...
def scan_download(download_url, conf):
	...
	try:
		...
		# if the file can be read, then Defender won't detect it as virus with the scan_data either
